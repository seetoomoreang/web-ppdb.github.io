        ...
        if($status=="Siswa")
            {
                ...
                $isi="
                    <table style='width:100%'>
                        ...
                        // Jalankan query
                        $result = mysqli_query($conn, "SELECT dana_pendidikan, infaq, spp, angsuran1, tgl_angsuran1, angsuran2, tgl_angsuran2, angsuran3, tgl_angsuran3, angsuran4, tgl_angsuran4 FROM tb_pembayaran WHERE id_siswa='$u'");
                        
                        if (!$result) {
                            die("Query gagal: " . mysqli_error($conn));
                        }
                        
                        $tp_dana = mysqli_fetch_row($result);
                        if (!$tp_dana) {
                            die("Data tidak ditemukan untuk ID siswa: $u");
                        }
                        
                        // Pastikan setiap elemen array tidak null
                        $dn_pendidikan = !empty($tp_dana[0]) ? preg_replace("/[^0-9]/", "", $tp_dana[0]) : 0;
                        $spp = !empty($tp_dana[2]) ? preg_replace("/[^0-9]/", "", $tp_dana[2]) : 0;
                        $inf = !empty($tp_dana[1]) ? preg_replace("/[^0-9]/", "", $tp_dana[1]) : 0;
                        
                        $jum_tot = number_format($dn_pendidikan + $spp + $inf, 0);
                        
                        $angsuran1 = !empty($tp_dana[3]) ? preg_replace("/[^0-9]/", "", $tp_dana[3]) : 0;
                        $jum = number_format(($dn_pendidikan + $spp + $inf) - $angsuran1, 0);
                        
                        $sp2 = !empty($tp_dana[2]) ? number_format($tp_dana[2], 0) : '0';
                        
                        // Output atau gunakan nilai
                        echo "Jumlah Total: $jum_tot";
